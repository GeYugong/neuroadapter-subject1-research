...
## 最新结果：T1/T2新训练已完成（2026-09-17）

两条从原canonical初始化的新训练、六份新snapshot与四份旧模型的500图双候选评价、最佳新候选导出及固定32图实际AI查看已全部完成。最佳新候选为 **T2-159375**，语义综合分85.168003，旧239063为85.891450。局部内容改善与失败同时存在，不能宣布重建修复或超过旧模型。新旧权重均保留，不再自动训练，不覆盖旧研究锁，不上传新权重HF。

完整结论、八指标、旧新差值、加载命令和32条视觉观察见[最终报告](docs/RETRAIN_LR_V1_RESULT.md)。本地含全部图片的版本为`artifacts/retrain-lr-v1/REPORT_ZH.md`；小型证据见`manifests/retrain-lr-v1/final/`。下方旧阶段描述属于历史背景，以本节及主日志最新记录为准。
...
当前已完成**三个指定权重的泛化诊断 v2**，不训练、不重新选优。既有 239063 步权重仍保留，不能因完成选优就视为已通过正式研究的重建性能验收。

## 当前阶段边界

**2026-09-15：A/B 诊断完成。** 回放 106250、159375、239063，复用旧噪声与 256 张图，新增 512 张图及 3840 对去噪比较；64 图缓存独立重建完全一致，完成 9000 图训练池的信号审查。训练 CLIP 持续提高，内部验证语义表现平台化，不能认定显著退化；验证池整体重复质量较差的解释暂未获支持。下一项优先为待授权的低学习率配对对照，本轮不启动训练、不触发 C 探针。结果见 [泛化诊断 v2 报告](docs/GENERALIZATION_DIAGNOSIS_V2.md)，完整 16 页图册位于 `artifacts/generalization-diagnosis-v2/REPORT.md`。

**2026-09-14 更新：允许受控诊断推理，不恢复训练。** 固定 32 张训练图和 32 张既有内部验证图，检查 VAE 往返、正确/整条错配 fMRI 的去噪误差与生成，再按前序结果决定是否执行同噪声的 FP32 和 guidance 对照。保留所有样本和失败记录，不按 GT 逐图选参数，不接触独立测试集。代码入口为 `scripts/diagnose_condition_path.py`，运行目录为项目根下 `runs/diagnostics/20260914-condition-path`；连续记录仍在 `EXPERIMENT_LOG.md`。

本轮已完成，结果见 [诊断报告](docs/CONDITION_DIAGNOSIS_20260914.md)。VAE 通路未见明显异常，正确 fMRI 有目标相关作用，但验证语义优势弱于训练；FP32 与 guidance=2/6 未给出明显整体修复。保持原权重和默认配置，不继续训练。含全部图片的本地报告为 `artifacts/diagnosis-20260914/REPORT.md`，public Git 仅保留统计和哈希索引。
