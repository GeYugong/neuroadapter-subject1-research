...
本轮已完成，不再训练。两条各159375更新、六新四旧500图双候选评价、最佳新权重导出、固定32图实际AI查看全部结束。结果及逐图观察见[最终报告](RETRAIN_LR_V1_RESULT.md)，本地完整含图版为`artifacts/retrain-lr-v1/REPORT_ZH.md`。

最佳新候选T2-159375，语义综合分85.168003，未超过旧239063的85.891450；相对旧同步数的+0.371075个百分点只是点估计，八项差值区间均跨零。不能宣布重建修复。权重已独立哈希和双候选重新加载复验；SHA256为`bc09013f3d75098d1b0e945e141e15f76b75d7aa6fcc0f68ace6b28815e0d259`，位置`runs/experiments/retrain-lr-v1/best_new_candidate/model.pt`。所有旧资产与研究锁保留，GPU计算任务已释放，无新增训练或HF上传。以下记录保留阶段历史。
...
首次方案试图只SIGSTOP控制器，但tmux会自动SIGCONT，因此已撤销；首次尝试保留在priority-T1/attempt-initial/。当前waiting_T2阶段原控制器和T2均保持运行。若调度器死亡，先确认其GPU评价子进程已退出；仅在两条训练完成且原控制器不存在时恢复冻结控制器，不能重启T2或另起训练。恢复会话为neuroadapter-retrain-lr-resumed。禁止改冻结runtime。
